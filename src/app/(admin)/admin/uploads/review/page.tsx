...
} from "@/components/smart-upload/confidence-indicator";
import type {
  ParsedPartRecord,
  ParseStatus,
  SecondPassStatus,
...
const REVIEW_FILTERS: Array<{
...
}> = [
  {
    value: "ACTIONABLE",
    label: "Actionable",
    description: "Processing, failed, and review-needed uploads",
  },
  {
    value: "NEEDS_REVIEW",
    label: "Needs Review",
    description: "Uploads ready for manual approval/rejection",
  },
  {
    value: "PROCESSING",
    label: "Processing",
    description: "Uploads still moving through workers or auto-commit",
  },
  {
    value: "FAILED",
    label: "Failed",
    description: "Uploads that failed and need operator attention",
  },
  {
    value: "APPROVED",
    label: "Approved",
    description: "Committed or approved sessions",
  },
  { value: "REJECTED", label: "Rejected", description: "Rejected sessions" },
  {
    value: "ALL",
    label: "All",
    description: "Every review-visible upload state",
  },
];
...
function getParseStatusBadge(parseStatus: ParseStatus | null): React.ReactNode {
  switch (parseStatus) {
    ...
    default:
      return (
        <Badge className="bg-yellow-100 text-yellow-700">Not Parsed</Badge>
      );
  }
}

function getSecondPassStatusBadge(
  secondPassStatus: SecondPassStatus | null,
): React.ReactNode {
...
function UploadReviewClient({
  ...
  const [sessions, setSessions] =
    useState<SmartUploadSession[]>(initialSessions);
  ...
  const [editingSession, setEditingSession] =
    useState<SmartUploadSession | null>(null);
  ...
  const [previewImages, setPreviewImages] = useState<
    Record<
      string,
      { imageBase64: string; totalPages: number; mimeType: string } | null
    >
  >({});
  ...
  // Debounce timer refs
  const originalPreviewDebounceRef = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  const partPreviewDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  // Part preview state
  const [selectedPart, setSelectedPart] = useState<ParsedPartRecord | null>(
    null,
  );
  const [partPreviewImages, setPartPreviewImages] = useState<
    Record<
      string,
      { imageBase64: string; totalPages: number; mimeType: string } | null
    >
  ...
  const [triggeringSecondPass, setTriggeringSecondPass] = useState<Set<string>>(
    new Set(),
  );
  ...
  const [statusFilter, setStatusFilter] =
    useState<ReviewStatusFilter>("ACTIONABLE");
  ...
  const fetchSessions = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/uploads/review?${params.toString()}`,
        {
          cache: "no-store",
        },
      );
  ...
  const canBulkSelectSession = (session: SmartUploadSession) => {
    return (
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedSessions(
        new Set(sessions.filter(canBulkSelectSession).map((s) => s.id)),
      );
    } else {
  ...
  const handleApprove = async (session: SmartUploadSession) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/uploads/review/${session.id}/approve`,
        {
          method: "POST",
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/uploads/review/${editingSession.id}/draft`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/uploads/review/${rejectSessionId}/reject`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const loadPreviewImage = useCallback(
    async (sessionId: string, page: number = 0, fmt?: "png" | "jpeg") => {
        ...
        try {
          ...
          if (res.ok) {
            ...
            setPreviewImages((prev) => ({
              ...prev,
              [sessionId]:
                data.imageBase64 && data.totalPages
                  ? {
  ...
  const loadPartPreviewImage = useCallback(
    async (
      sessionId: string,
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-5">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats.actionable}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
      ...
      {!focusedSessionId && (
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Review Queue Visibility</CardTitle>
            <CardDescription>
              {activeFilter?.description ??
                "Showing all actionable Smart Upload sessions."}
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {focusedSessionId
              ? "Focused Upload Session"
              : `${activeFilter?.label ?? "Actionable"} Uploads`}
          </CardTitle>
          <CardDescription>
            {focusedSessionId
              ? "Review the upload session opened from Smart Upload. Refresh to see processing updates."
              : "The default queue now includes processing, failed, and review-needed sessions so uploads do not disappear while workers run."}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {sessions.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">
                {focusedSessionId
                  ? "This upload session was not found or you do not have access to it."
                  : "Try another queue filter, or verify the Smart Upload worker is running if a new upload appears stuck."}
              </p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-[50px]">
                    <Checkbox
                      checked={
                        sessions.filter(canBulkSelectSession).length > 0 &&
                        selectedSessions.size ===
                          sessions.filter(canBulkSelectSession).length
                      }
      ...
      >
        <DialogContent
          className={cn(
            ...
            isFullscreen && "max-w-none h-screen m-0 rounded-none",
          )}
        >
            ...
            <div className="space-y-6 flex-1 overflow-y-auto pr-1">
              ...
              <Tabs defaultValue="original" className="w-full">
                <TabsList>
                  ...
                  {editingSession.parsedParts &&
                    editingSession.parsedParts.length > 0 && (
                ...
                <TabsContent value="original" className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      {editingSession.exceptionQueue?.original && (
                        <>
                          <Button variant="outline" size="sm" asChild>
                            <a
                              href={
                                editingSession.exceptionQueue.original.links
                                  .openPath
                              }
                              target="_blank"
                              rel="noreferrer"
                      ...
                      <span className="text-sm">
                        Page {totalPages > 0 ? currentPage + 1 : 0} /{" "}
                        {totalPages}
                      </span>
                      <Button
                        variant="outline"
                  ...
                  previewImages[editingSession.id] === undefined ? (
                    ...
                    <div className="w-full h-20 bg-muted rounded-lg flex flex-col items-center justify-center gap-2 border border-dashed">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          editingSession &&
                          loadPreviewImage(editingSession.id, currentPage)
                        }
                      >
                        Retry
                      </Button>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 mb-4">
                        {editingSession.parsedParts.map((part, index) => (
                          <Button
                            key={index}
                            variant={
                              selectedPart === part ? "default" : "outline"
                            }
                            size="sm"
                          ...
                          >
                            ...
                            <span className="text-xs opacity-70">
                              {sanitizeDisplayValue(part.instrument)}
                            </span>
                            <span className="text-xs opacity-50">
                              {formatHumanRange(
                                part.pageRange as [number, number],
                              )}{" "}
                        ...
                        <>
                          <div className="flex items-center justify-between">
                            <h4 className="text-sm font-semibold">
                              ...
                              {selectedPart.instrument})
                            </h4>
                            <div className="flex items-center gap-2">
                              {editingSession.exceptionQueue?.parts &&
                                (() => {
                                  const selectedPartLinks =
                                    editingSession.exceptionQueue?.parts.find(
                                      (part) =>
                                        part.storageKey ===
                                        selectedPart.storageKey,
                                    )?.links;
                                  return selectedPartLinks ? (
                                    <>
                                      ...
                                      >
                                        <a
                                          href={selectedPartLinks.openPath}
                                          target="_blank"
                                          rel="noreferrer"
                                        >
                                          <ExternalLink className="mr-1 h-4 w-4" />
                                      ...
                                      >
                                        <a
                                          href={selectedPartLinks.downloadPath}
                                        >
                                          <Download className="mr-1 h-4 w-4" />
                                          Download Part
                                        </a>
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handlePartPageChange(partCurrentPage - 1)
                                }
                                disabled={
                                  partCurrentPage === 0 || partTotalPages === 0
                                }
                              >
                                <ChevronLeft className="h-4 w-4" />
                              </Button>
                              <span className="text-sm">
                                Page{" "}
                                {partTotalPages > 0 ? partCurrentPage + 1 : 0} /{" "}
                                {partTotalPages}
                              </span>
                              <Button
                                variant="outline"
                                ...
                                disabled={
                                  partCurrentPage >= partTotalPages - 1 ||
                                  partTotalPages === 0
                                }
                              >
                                <ChevronRight className="h-4 w-4" />
                              ...
                              <div className="flex items-center gap-1 ml-2">
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    handlePartZoomChange(partZoomLevel - 0.25)
                                  }
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  setIsPartFullscreen(!isPartFullscreen)
                                }
                                className="ml-2"
                              >
                                {isPartFullscreen ? (
              ...
              <div className="space-y-3">
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <span className="text-xs text-muted-foreground">
                    ...
                    {editingSession.extractedMetadata?.enforceOcrSplitting
                      ? "Yes"
                      : "No"}
                  </span>
                </div>
                {editingSession.confidenceScore !== null &&
                  editingSession.confidenceScore < 70 && (
                    <ConfidenceWarningBanner
                      score={editingSession.confidenceScore}
                      threshold={70}
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="rounded-lg border bg-muted/30 p-4">
                    ...
                    <div className="mt-3 text-xs text-muted-foreground">
                      Type:{" "}
                      {formatExceptionKind(editingSession.exceptionQueue.kind)}
                    </div>
                  </div>
                  <div className="rounded-lg border bg-muted/30 p-4">
                    ...
                    <div className="space-y-1 text-xs text-muted-foreground">
                      <div>
                        OCR engine:{" "}
                        {editingSession.exceptionQueue.provenance
                          .ocrEngineUsed || "none recorded"}
                      </div>
                      <div>
                        OCR chars:{" "}
                        {editingSession.exceptionQueue.provenance
                      ...
                      <div>
                        Split source:{" "}
                        {editingSession.extractedMetadata
                          ?.cuttingInstructionsSource ?? "unknown"}
                      </div>
                      <div>
                        Strategy attempts:{" "}
                        {editingSession.exceptionQueue.provenance
                          .strategyHistoryCount ?? 0}
                      </div>
                      {editingSession.exceptionQueue.provenance
                        .llmFallbackReasons &&
                        editingSession.exceptionQueue.provenance
                          .llmFallbackReasons.length > 0 && (
                          <div>
                            LLM fallback:{" "}
                            {editingSession.exceptionQueue.provenance.llmFallbackReasons.join(
                              "; ",
                            )}
                          </div>
                        )}
              ...
              {/* Gap Warning */}
              {editingSession.cuttingInstructions?.some(
                (inst) => (inst.partNumber ?? 0) >= 9900,
              ) && (
                  ...
                  <div>
                    ...
                    <p className="text-xs mt-0.5">
                      ...
                      {editingSession.cuttingInstructions
                        ...
                        .map(
                          (inst) =>
                            `pages ${formatHumanRange(inst.pageRange as [number, number])}`,
                        )
                        .join(", ")}
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEditedMetadata({
                        ...editedMetadata,
                        fileType: e.target
                          .value as ExtractedMetadata["fileType"],
                      })
                    }
                ...
                editingSession.extractedMetadata.parts && (
                  <div className="space-y-2">
                    ...
                    <div className="bg-muted p-3 rounded-lg max-h-56 overflow-y-auto">
                      {editingSession.extractedMetadata.parts.map(
                        (part, index) => (
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-yellow-700 mt-1">
                                On approval, the original PDF will be stored as
                                a single file. You can manually trigger
                                splitting after running the second-pass
                                analysis.
                              </p>
                            </div>
                          </div>
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-yellow-700 mt-1">
                            On approval, the original PDF will be stored as a
                            single file. You can manually trigger splitting
                            after running the second-pass analysis.
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            <div className="flex items-center gap-2 mr-auto">
              ...
              <Button variant="outline" size="sm" asChild>
                <a
                  href={
                    editingSession
                      ? `/api/admin/uploads/review/${editingSession.id}/original?disposition=attachment`
                      : "#"
                  }
                  download
                >
      ...
      <Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to reject this upload? This action cannot be
              undone.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to reject {selectedSessions.size} selected
              upload(s)? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
