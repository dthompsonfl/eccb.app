 ...
 */
function validateJsonFields(settings: Array<{ key: string; value: string }>): {
  valid: boolean;
  error?: string;
} {
 ...
 */
function settingsArrayToRecord(
...
export async function GET() {
  try {
    ...
    const rows: SystemSetting[] = await prisma.systemSetting.findMany({
    ...
    const snapshot = await loadSmartUploadSettingsSnapshot();

    return NextResponse.json({
      settings: settingsArray,
      canonicalSource: buildSmartUploadSettingsSnapshotSummary(snapshot),
      deprecatedSources: ["SmartUploadSetting"],
    });
  } catch (error) {
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const { settings: existingSettings } =
      await loadSmartUploadSettingsFromDB();
    ...
    const mergedRecord = mergeSettingsPreservingSecrets(
      existingSettings,
      incomingRecord,
    );
