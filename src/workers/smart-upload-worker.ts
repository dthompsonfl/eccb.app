...
async function convertAllPdfPagesToImages(
  ...
  if (totalPages <= effectiveMax) {
    ...
    logger.warn(
      "Second-pass rendering: sampling pages due to provider image limit",
      {
        totalPages,
        maxImages,
        effectiveMax,
  ...
  logger.info("Converting PDF pages to images", {
    totalPages,
    pagesToProcess: pageIndices.length,
  });
...
async function callVerificationLLM(
  ...
  const verificationStepConfig = await buildAdapterConfigForStep(
    cfg,
    "verification",
  );
  const adapterConfig = {
  ...
  const response = await callVisionModel(adapterConfig, images, prompt, {
    system:
      verificationStepConfig.systemPrompt || DEFAULT_VERIFICATION_SYSTEM_PROMPT,
...
function detectDisagreements(
  ...
  if (
    first.title?.toLowerCase().trim() !== second.title?.toLowerCase().trim()
  ) {
    disagreements.push(`Title mismatch: "${first.title}" vs "${second.title}"`);
  }

  if (
    first.composer?.toLowerCase().trim() !==
    second.composer?.toLowerCase().trim()
  ) {
    disagreements.push(
      `Composer mismatch: "${first.composer}" vs "${second.composer}"`,
    );
  }

  // Compare cutting instructions (instrument mapping)
  const firstParts =
    first.cuttingInstructions
      ?.map((p) => p.instrument)
      .sort()
      .join(",") || "";
  const secondParts =
    second.cuttingInstructions
      ?.map((p) => p.instrument)
      .sort()
      .join(",") || "";
...
async function callAdjudicatorLLM(
  ...
  const adjudicatorStepConfig = await buildAdapterConfigForStep(
    cfg,
    "adjudicator",
  );
  const adapterConfig = {
  ...
  if (pdfDocuments && pdfDocuments.length > 0) {
    ...
    const adjStep = Math.max(
      1,
      Math.floor(pageImages.length / MAX_ADJUDICATOR_PAGES),
    );
    const adjudicatorPageImages = pageImages
      .filter((_, i) => i % adjStep === 0)
      .slice(0, MAX_ADJUDICATOR_PAGES);
  ...
  const response = await callVisionModel(adapterConfig, images, prompt, {
    system:
      adjudicatorStepConfig.systemPrompt || DEFAULT_ADJUDICATOR_SYSTEM_PROMPT,
    responseFormat: { type: "json" as const },
    maxTokens: 65536,
    temperature: 0.1,
...
function parseVerificationResponse(content: string): ExtractedMetadata {
  ...
  if (!result.ok) {
    logger.warn(
      "parseVerificationResponse: JSON extraction failed, returning low-confidence fallback",
      { error: result.error },
    );
...
async function finalizeSmartUploadSession(
  ...
  const firstPassMeta = parseSmartUploadJsonField<ExtractedMetadata | null>(
    smartSession.extractedMetadata,
    null,
  );
  const firstPassSegConf = firstPassMeta?.segmentationConfidence;
  const hasSecondPassInstructions =
    (finalMetadata.cuttingInstructions?.length ?? 0) > 0;
  ...
  if (allGarbage && isScoreFileType) {
    ...
    if (totalPages <= MAX_FULL_SCORE_PAGES) {
      ...
      logger.info(
        "Second pass: single-document score — replaced garbage instructions with full-score",
        {
          sessionId,
          scoreType,
          totalPages,
  ...
  if (correctedCuttingInstructions && correctedCuttingInstructions.length > 0) {
    try {
      ...
      const zeroIndexed = oneIndexed.map((ins) => ({
        ...ins,
        pageRange: [ins.pageRange[0] - 1, ins.pageRange[1] - 1] as [
          number,
          number,
        ],
      }));
      ...
      if (gapInstructions.length > 0) {
        ...
        const oneIndexedGaps = gapInstructions.map((g) => ({
          ...g,
          pageRange: [g.pageRange[0] + 1, g.pageRange[1] + 1] as [
            number,
            number,
          ],
        }));
        logger.warn(
          "Second pass gap detection: adding unlabelled parts for uncovered pages",
          {
            sessionId,
        ...
        if (largeGap) {
          ...
          logger.warn(
            "Large unlabelled gap detected — marking session for human review",
            {
              sessionId,
              largestGap: Math.max(
                ...oneIndexedGaps.map(
                  (g) => g.pageRange[1] - g.pageRange[0] + 1,
                ),
              ),
            },
          );
  ...
  if (correctedCuttingInstructions && correctedCuttingInstructions.length > 0) {
    ...
    correctedCuttingInstructions = sanitizeCuttingInstructionsForSplit(
      correctedCuttingInstructions,
    );
  ...
  const isParsed =
    updateData.parseStatus === "PARSED" ||
    smartSession.parseStatus === "PARSED";

  // ── Shared Quality Gates ───────────────────────────────────────────────
  if (!updateData.requiresHumanReview) {
    const parts: ParsedPartRecord[] =
      (updateData.parsedParts as ParsedPartRecord[] | undefined) ??
      parsedParts ??
      [];
  ...
  if (
    ...
    logger.info("Session auto-approved after processing (legacy threshold)", {
      sessionId,
      finalConfidence,
    });
  }
...
async function processSecondPass(
  job: Job<SmartUploadSecondPassJobData>,
): Promise<void> {
  ...
  const currentSecondPassStatus =
    smartSession.secondPassStatus as SecondPassStatus;
  ...
  if (
    ...
    throw new Error(
      `Session is not eligible for second pass. Current status: ${currentSecondPassStatus}`,
    );
  }
  ...
  if (smartSession.routingDecision === "no_parse_second_pass") {
    logger.warn(
      "Skipping second-pass for session with gaps detected in first pass",
      {
        sessionId,
    ...
    await progress(
      "skipped",
      100,
      "Gaps detected in first pass; routing to human review",
    );
  ...
  try {
    // Load Smart Upload runtime config through the canonical facade.
    ...
    if (canSendPdf) {
      ...
      logger.info(
        "Second pass: using PDF-to-LLM mode — skipping image rendering",
        {
          sessionId,
      ...
      await progress(
        "rendering",
        35,
        "PDF prepared for AI analysis (native mode)",
      );
    } else {
      ...
      const providerImageCap =
        providerMeta?.maxImagesPerRequest ?? MAX_PDF_PAGES_FOR_LLM;
      ...
      if (totalPageCount > effectiveMaxImages) {
        logger.warn(
          "Second pass: total pages exceed provider image cap — sampling",
          {
            sessionId,
            totalPageCount,
            effectiveMaxImages,
      ...
      originalPageImages = await renderPdfPageBatch(
        originalPdfBuffer,
        pageIndices,
        {
          scale: 2,
    ...
    if (
        ...
        if (!canSendPdf) {
          try {
            ...
            if (typeof partDownloadResult !== "string") {
              const partPdfBuffer = await streamToBuffer(
                partDownloadResult.stream,
              );
              const partPageImages =
                await convertAllPdfPagesToImages(partPdfBuffer);
      ...
      verificationPrompt =
        buildVerificationPrompt(
          llmConfig.verificationUserPrompt ||
      ...
      try {
        ...
        const verificationConfig = await buildAdapterConfigForStep(
          llmConfig,
          "verification",
        );

        logger.error(
          "Verification LLM call failed — check provider configuration for fallback",
          {
            sessionId,
            ...
            recommendation:
              "Operator should retry with alternative provider or manually review",
          },
        );
      ...
      const verificationConfidence = normalizeConfidence(
        (secondPassResult as unknown as Record<string, unknown>)
          .verificationConfidence ?? secondPassResult.confidenceScore,
      );
      ...
      if (
        ...
        logger.warn(
          "Second pass returned empty/garbage cutting instructions — preserving first-pass instructions",
          {
            sessionId,
      ...
      const lowConfidence =
        verificationConfidence < 85 || metadata.confidenceScore < 80;
      // Re-evaluate garbage state after fallback
      const finalSpCuts = secondPassResult.cuttingInstructions || [];
      const stillGarbage =
        finalSpCuts.length === 0 ||
        finalSpCuts.every(
          (ci) =>
            isForbiddenLabel(ci.instrument) || isForbiddenLabel(ci.partName),
        );
      const needsAdjudication =
        disagreements.length > 0 || lowConfidence || stillGarbage;
      ...
      if (needsAdjudication) {
        ...
        const adjudicatorPrompt = buildAdjudicatorPrompt(
          llmConfig.adjudicatorUserPrompt || llmConfig.adjudicatorPrompt || "",
          {
            ...
            secondPassMetadata: secondPassResult as unknown as Record<
              string,
              unknown
            >,
            disagreements,
        ...
        try {
          ...
          const adjConfig = await buildAdapterConfigForStep(
            llmConfig,
            "adjudicator",
          );
          logger.error(
            "Adjudicator LLM call failed — check provider configuration",
            {
              sessionId,
              provider: adjConfig.provider,
              model: adjConfig.model,
              error:
                adjError instanceof Error ? adjError.message : String(adjError),
            },
          );
          throw adjError;
        }
        ...
        if (finalConfidence === 0 && verificationConfidence > 0) {
          logger.warn(
            "Adjudicator produced zero-confidence result — falling back to verification result",
            {
              sessionId,
...
Include a "corrections" field explaining any corrections made from the first pass, or null if no corrections were needed.`;
      ...
      if (
        ...
        logger.warn(
          "Second pass (fallback) returned empty/garbage cutting instructions — preserving first-pass instructions",
          {
            sessionId,
            secondPassCuts: spCuts.length,
            firstPassCuts: cuttingInstructions.length,
      ...
      const needsAdjudication =
        disagreements.length > 0 || verificationConfidence < 85 || stillGarbage;
    ...
    recordMetricError(
      sessionId,
      SmartUploadErrorCode.VERIFY_LLM_FAILED,
      "verification",
      duration,
    );
    ...
    await prisma.smartUploadSession.update({
      ...
      data: {
        ...
        requiresHumanReview: true,
        routingDecision: smartSession.routingDecision || "SECOND_PASS_FAILED",
      },
    });
